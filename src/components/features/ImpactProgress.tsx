...
export function ImpactProgress({ project, compact = false, className }: Props) {
  ...
  return (
    <div className={cn('space-y-4', className)}>
      {/* Visual B&N → color */}
      <div className="relative overflow-hidden rounded-2xl border border-border bg-gradient-to-b from-eco-50 to-card">
        {/* base en escala de grises */}
        <div className="grayscale [filter:grayscale(1)] opacity-25">
          <ProductIllustration type={project.illustration} className="block h-auto w-full" />
        </div>
        {/* capa a color, revelada de abajo hacia arriba */}
        <motion.div
        ...
        <div className="absolute right-3 top-3 rounded-full bg-card/90 px-3 py-1 text-sm font-extrabold text-eco-700 shadow-sm backdrop-blur">
          {total}%
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="rounded-xl bg-eco-50 p-3">
          <div className="flex items-center gap-1.5 text-xs font-semibold text-eco-700">
            <Sparkles size={14} /> Tu aporte
          </div>
          <p className="mt-1 text-lg font-extrabold text-eco-800 leading-none">
