...
export default function Ranking() {
  ...
  const { data, loading } = useApi(() => api.getRanking(scope), [scope])
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Premio del mes */}
      {data && (
        <div className="flex items-center gap-3 rounded-2xl border border-xp-200 bg-xp-50 p-4">
          <span className="text-3xl">{data.prize.emoji}</span>
          <div>
            <p className="text-xs font-semibold uppercase tracking-wide text-xp-700">
